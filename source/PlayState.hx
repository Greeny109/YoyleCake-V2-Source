...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Oobily Goobily Weezer Beezers', 0.2], //From 0% to 19%
		['Thats Terrible!', 0.4], //From 20% to 39%
		['Vengeful', 0.5], //From 40% to 49%
		['Hurtful', 0.6], //From 50% to 59%
		['Almost Passable', 0.69], //From 60% to 68%
		['Noice', 0.7], //69%
		['Yoy', 0.8], //From 70% to 79%
		['Yoyle', 0.9], //From 80% to 89%
		['Yoylecake!', 1], //From 90% to 99%
		['Wowzer berwowzer!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		var offsetTimerY:Float = 25; 

		var showTime:Bool = (ClientPrefs.timeBarType != 'Disabled');
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 19, 400, "", 32);
		timeTxt.setFormat(Paths.font("SevenSegment.ttf"), 32, FlxColor.fromRGB(250, 198, 18), CENTER);
		timeTxt.scrollFactor.set();
		timeTxt.alpha = 0;
		timeTxt.borderSize = 0;
		timeTxt.visible = showTime;
		timeTxt.size = 75;
		timeTxt.y += offsetTimerY;
		if(ClientPrefs.downScroll) timeTxt.y = FlxG.height - 110;

		var showTimerBack:Bool = (ClientPrefs.timeBarType != 'Disabled');
		timerBack = new FlxSprite(100).loadGraphic(Paths.image('boob'));
		timerBack.screenCenter(X);
		timerBack.y += -335 + offsetTimerY;
		timerBack.x += -5;
		timerBack.visible = showTimerBack;
		timerBack.scrollFactor.set();
		timerBack.scale.x = 0.12;
		timerBack.scale.y = 0.12;
		if(ClientPrefs.downScroll) timerBack.y = 0.35 * FlxG.height;
		add(timerBack);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			case 'Color_Flash_Camera':
				var val1:Array<String> = value1.split(',');
				var val2:Array<String> = value2.split(',');

				var theCam:FlxCamera = cameraFromString(val1[0]);
				var forced:Bool = if (val1[1].trim().toLowerCase() == 'true') true else false;
				var theColor:FlxColor = FlxColor.fromString('#' + val2[0].trim());
				trace(val2[0]);
				var theDuration:Float = Std.parseFloat(val2[1].trim());
				var theAlpha:Float = Std.parseFloat(val2[2].trim());

				theColor.alphaFloat = theAlpha;
				theCam.flash(theColor, theDuration, null, forced);
