...
class TitleState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		var pressedEnter:Bool = true;//FlxG.keys.justPressed.ENTER || controls.ACCEPT;
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxG.camera.flash(ClientPrefs.flashing ? FlxColor.BLACK : 0x4CFFFFFF, 1);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if PSYCH_WATERMARKS
					createCoolText(['vs BFDI yoylecake by...'], 15);
					#else
					createCoolText(['Dangerbites', 'uhmmm', '', 'sebota i dink']);
					#end
